...
func makeStatusHandler(m *mux.Router, endpoints endpoint.Endpoints, options []http.ServerOption) {
	m.Methods("GET").Path("/status").Handler(handlers.CORS(handlers.AllowedMethods([]string{"GET"}), handlers.AllowedOrigins([]string{"*"}))(http.NewServer(endpoints.StatusEndpoint, decodeStatusRequest, encodeStatusResponse, options...)))
}
...
// makeValidateHandler creates the handler logic
func makeValidateHandler(m *mux.Router, endpoints endpoint.Endpoints, options []http.ServerOption) {
	m.Methods("POST").Path("/validate").Handler(handlers.CORS(handlers.AllowedMethods([]string{"POST"}), handlers.AllowedOrigins([]string{"*"}))(http.NewServer(endpoints.ValidateEndpoint, decodeValidateRequest, encodeValidateResponse, options...)))
}

// decodeValidateRequest is a transport/http.DecodeRequestFunc that decodes a
// JSON-encoded request from the HTTP request body.
func decodeValidateRequest(_ context.Context, r *http1.Request) (interface{}, error) {
	req := endpoint.ValidateRequest{}
	err := json.NewDecoder(r.Body).Decode(&req)
	return req, err
}

// encodeValidateResponse is a transport/http.EncodeResponseFunc that encodes
// the response as JSON to the response writer
func encodeValidateResponse(ctx context.Context, w http1.ResponseWriter, response interface{}) (err error) {
	if f, ok := response.(endpoint.Failure); ok && f.Failed() != nil {
		ErrorEncoder(ctx, f.Failed(), w)
		return nil
	}
